...
const char * const BITMONKEY_CONF_FILENAME = "bitmonkey.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitmonkeyd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\bitmonkey
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\bitmonkey
    // Mac: ~/Library/Application Support/bitmonkey
    // Unix: ~/.bitmonkey
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITMONKEY_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure bitmonkey Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("bitmonkey Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The bitmonkey Core developers";
    }
