...
{
    // Call into TxToUniv() in bitmonkey-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in bitmonkey-common, so we query them here and push the
...
static UniValue getrawtransaction(const JSONRPCRequest& request)
{
    const RPCHelpMan help{
            ...
            "           \"address\"        (string) bitmonkey address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
static UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    const RPCHelpMan help{"decoderawtransaction",
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) bitmonkey address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue decodepsbt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "          \"hex\" : \"hex\",            (string) The hex\n"
            "          \"type\" : \"pubkeyhash\",    (string) The type, eg 'pubkeyhash'\n"
            "          \"address\" : \"address\"     (string) bitmonkey address if there is one\n"
            "        }\n"
            "      },\n"
