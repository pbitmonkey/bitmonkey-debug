 ...
 */
static const char* BITMONKEY_PID_FILENAME = "bitmonkeyd.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITMONKEY_PID_FILENAME)));
}
 ...
 * between bitmonkeyd, and bitmonkey-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", BITMONKEY_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", BITMONKEY_PID_FILENAME), false, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitmonkey/bitmonkey>";
    const std::string URL_WEBSITE = "<https://bitmonkeycore.org>";
...
/** Sanity checks
 *  Ensure that bitmonkey is running in a usable environment with all
...
{
    // Make sure only a single bitmonkey process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", BITMONKEY_CONF_FILENAME));
