...
const int BITMONKEY_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BITMONKEY_IPC_PREFIX("bitmonkey:");
#ifdef ENABLE_BIP70
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/bitmonkey-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/bitmonkey-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/bitmonkey-paymentrequest";
#endif
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the bitmonkey: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(BITMONKEY_IPC_PREFIX, Qt::CaseInsensitive)) // bitmonkey: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parsebitmonkeyURI(arg, &r) && !r.address.isEmpty())
            {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(BITMONKEY_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(BITMONKEY_IPC_CONNECT_TIMEOUT);
...
#endif
{
    ...
    // on Mac: sent when you click bitmonkey: links
...
//
// OSX-specific way of handling bitmonkey: URIs and PaymentRequest mime types.
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    else if (s.startsWith(BITMONKEY_IPC_PREFIX, Qt::CaseInsensitive)) // bitmonkey: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parsebitmonkeyURI(s, &recipient))
            {
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in bitmonkey: URIs
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom bitmonkey addresses are not supported
        ...
        // bitmonkey amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(bitmonkeyUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
