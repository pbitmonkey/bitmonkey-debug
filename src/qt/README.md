...
ClientModel is used by the main application `bitmonkeygui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/bitmonkey/bitmonkey/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a bitmonkey:... link from a browser).
...
* UI elements like bitmonkeyAmountField, which inherit from QWidget.
* `bitmonkeystrings.cpp`: automatically generated
* `bitmonkeyunits.(h/cpp)`: BTM / mBTM / etc handling
* `callback.h`
...
* don't change `local/bitmonkey_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
