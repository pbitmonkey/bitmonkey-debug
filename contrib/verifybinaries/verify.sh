#!/usr/bin/env bash
# Copyright (c) 2016 The bitmonkey Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   bitmonkeycore.org and bitmonkey.org and compares them.
...
WORKINGDIR="/tmp/bitmonkey_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitmonkeycore.org"
HOST2="https://bitmonkey.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitmonkey-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitmonkey-',
...
if [ $? -ne 0 ]; then
   echo "bitmonkey.org failed to provide signature file, but bitmonkeycore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitmonkey.org and bitmonkeycore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the bitmonkey Core binary release signing key installed?"
   fi
