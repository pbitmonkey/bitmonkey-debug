bitmonkey Core version 0.14.0 is now available from:

  <https://bitmonkey.org/bin/bitmonkey-core-0.14.0/>
...
bitmonkey Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/bitmonkey/bitmonkey/blob/master/doc/zmq.md) for
...
The Alert System was [disabled and deprecated](https://bitmonkey.org/en/alert/2016-11-01-alert-retirement) in bitmonkey Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   ...
   optimizations to memory management. See [Pull #8753](https://github.com/bitmonkey/bitmonkey/pull/8753)
...
- #8722 `bb843ad` bitmonkey-cli: More detailed error reporting (laanwj)
...
- #7562 `1eef038` Bump transaction version default to 2 (btmdrak)
...
- #8608 `7e9ab95` Install manpages via make install, also add some autogenerated manpages (nomnombtm)
...
- #8568 `078900d` new var `DIST_CONTRIB` adds useful things for packagers from contrib (nomnombtm)
...
- #8830 `ef0801b` test: Add option to run bitmonkey-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to bitmonkey-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to bitmonkey-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and bitmonkey-util-test.py (jnewbery)
...
- #9206 `e662d28` Make test constant consistent with consensus.h (btmdrak)
...
- #9691 `fc67cd2` Init ECC context for `test_bitmonkey_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change bitmonkey address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` bitmonkeyd: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow bitmonkey-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` bitmonkey-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to bitmonkey-tx (jnewbery)
