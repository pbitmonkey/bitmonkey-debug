bitmonkey Core version 0.18.0 is now available from:

  <https://bitmonkeycore.org/bin/bitmonkey-core-0.18.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://bitmonkeycore.org/en/list/announcements/join/>
...
`contrib/init/bitmonkeyd.service`, it has been changed to use
`/var/lib/bitmonkeyd` as the data directory instead of
`~bitmonkey/.bitmonkey`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/bitmonkeyd` will
exist has enough space (check using `df -h /var/lib/bitmonkeyd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/bitmonkey/bitmonkey/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitmonkeycore.org/en/contact/) the bitmonkey Core
...
The systemd init file (`contrib/init/bitmonkeyd.service`) has been changed
to use `/var/lib/bitmonkeyd` as the data directory instead of
`~bitmonkey/.bitmonkey`. This change makes bitmonkey Core more consistent with
...
`datadir` option in `/etc/bitmonkey/bitmonkey.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/bitmonkey/bitmonkey.conf`.


Documentation
-------------

- A new short [document](https://github.com/bitmonkey/bitmonkey/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/bitmonkey/bitmonkey/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/bitmonkey/bitmonkey/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/bitmonkey/bitmonkey/blob/master/doc/bitmonkey-conf.md)
...
- A new document introduces bitmonkey Core's BIP174 [Partially-Signed
  ...
  (PSBT)](https://github.com/bitmonkey/bitmonkey/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/bitmonkey/bitmonkey/blob/master/doc/descriptors.md)
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/bitmonkey/bitmonkey/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/bitmonkey/bitmonkey/blob/master/doc/descriptors.md).
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/bitmonkey/bitmonkey/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/bitmonkey/bitmonkey/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- Since version 0.16.0, bitmonkey Core’s built-in wallet has defaulted to
  ...
  addresses in bitmonkey Core 0.19 (approximately November 2019).
...
- A sub-project of bitmonkey Core now provides Hardware Wallet Interaction
  ...
  page](https://github.com/bitmonkey-core/HWI#readme) for details.
...
- #12490 Remove deprecated wallet rpc features from `bitmonkey_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbitmonkey_server.a` (jnewbery)
...
- #13926 [Tools] bitmonkey-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like bitmonkeyd (MarcoFalke)
...
- #14521 qt, docs: Fix `bitmonkey-qt -version` output formatting (hebasto)
...
- #11625 Add bitmonkeyApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbitmonkey_server` (laanwj)
...
- #15154 Configure: bitmonkey-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bitmonkey-wallet (Sjors)
...
- #11526 Visual Studio build configuration for bitmonkey Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbitmonkey_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in bitmonkey-util-test.py (isghe)
...
- #12775 Integration of property based testing into bitmonkey Core (Christewart)
...
- #12246 Bugfix: Only run bitmonkey-tx tests when bitmonkey-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_bitmonkey` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_bitmonkey` (MarcoFalke)
...
- #14186 bitmonkey-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_bitmonkey.conf` message on startup if conf file exists (leishman)
...
- #12255 Update bitmonkey.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bitmonkey-conf.md (hebasto)
...
- #15354 Add missing `bitmonkey-wallet` tool manpages (MarcoFalke)
...
- #15765 correct bitmonkeyconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support bitmonkey folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/bitmonkey/).
